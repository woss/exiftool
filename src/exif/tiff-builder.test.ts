...
import { buildTiff } from './tiff-builder.ts';
import { extractExifFromTiff } from './tiff.ts';

Deno.test('buildTiff writes ASCII tags readable by the TIFF parser', () => {
...
Deno.test('buildTiff encodes SHORT and RATIONAL values', () => {
...
Deno.test('buildTiff maps ModifyDate onto the DateTime field', () => {
...
Deno.test('buildTiff coerces booleans and plain numeric strings for ascii tags', () => {
...
Deno.test('buildTiff accepts plain numeric strings for rational tags', () => {
  const { bytes } = buildTiff({ XResolution: '350' });
  assertEquals(extractExifFromTiff(bytes).XResolution, 350);
});

Deno.test('buildTiff skips values that cannot encode to the target type', () => {
